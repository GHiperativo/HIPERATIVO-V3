  ...
  try {
    ...
    abas.forEach(function(aba) {
      ...
      if (!cel || String(cel).trim() === '') {
        // PROTEÇÃO CRÍTICA: só recria estrutura se não houver dados.
        // Se lastRow > 1, a aba tem conteúdo — chamar criarFn apagaria tudo.
        // Nesse caso registra o aviso mas NÃO toca nos dados.
        if (ws.getLastRow() > 1) {
          _log('SYSTEM', 'AVISO', 'restaurarEstrutura',
            aba.nome + ': header vazio mas aba tem ' + (ws.getLastRow() - 1) + ' linha(s) de dados. Pulando recreação para preservar dados.', '');
          corrigidas.push(aba.nome + ' (header vazio — DADOS PRESERVADOS, restaure header manualmente)');
        } else {
          try { aba.criarFn(ws); } catch(exFreeze) { if (String(exFreeze.message).indexOf("congelar") === -1 && String(exFreeze.message).indexOf("freeze") === -1) throw exFreeze; try { ws.setFrozenRows(2); } catch(e2) {} }
