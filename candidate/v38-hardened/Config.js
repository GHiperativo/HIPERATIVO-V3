...
function onOpen() {
  ...
  menu.addSubMenu(
    ui.createMenu('👤 Atletas')
      ...
      .addItem('🔗 Gerar links reconexão Strava (todos)', 'gerarLinksReconexaoTodos')
  );
  ...
  menu.addSubMenu(
    ui.createMenu('⚙️ Configurações')
      ...
      .addItem('🗂️ Reformatar cabeçalho do CADASTRO (27 cols)', 'reformatarCabecalhoCadastro')
  ...
  // ─── MANUTENÇÃO E SUPABASE ────────────────────────────────────────────
  menu.addSubMenu(
    ui.createMenu('🛠️ Manutenção')
      .addItem('✨ Aplicar todos os ajustes (recomendado)', 'ajustesPlanilha')
      .addSeparator()
      .addItem('🧹 Limpar linhas vazias — ATIVIDADES', 'limparAtividades')
      .addItem('🔐 Limpar tokens corrompidos — TOKENS', 'limparTokensCorrompidos')
      .addItem('📈 Limpar headers duplicados — MÉTRICAS', 'limparMetricasCorrompidas')
      .addItem('📊 Corrigir alertas de data — PAINEL', 'corrigirPainel')
      .addItem('👥 Corrigir contador de Ativos (Trial)', 'corrigirContadorAtivos')
      .addItem('⚙️ Popular aba CONFIG', 'popularConfig')
      .addItem('🔴 Limpar log de erros antigos', 'trimmarErrosPadrao')
      .addSeparator()
      .addItem('♻️ Restaurar atletas (emergência)', 'restaurarAtletas')
      .addItem('🗂️ Limpar abas desconhecidas', 'limparPlanilha')
      .addSeparator()
      .addItem('☁️ Restaurar tokens do Supabase', 'supaRestaurarTodosTokens')
      .addItem('📡 Status do Supabase', 'supaStatus')
      .addItem('🔑 Configurar SUPABASE_KEY', 'configurarSupabaseKey')
      .addSeparator()
      .addItem('🔬 Diagnóstico completo do sistema', 'diagnosticoSistema')
      .addItem('🌐 Corrigir URL do Web App (Strava)', 'fixarWebApp')
      .addItem('🔗 Definir nova URL do Web App', 'definirWebAppUrl')
  );

  // ─── STRAVA ──────────────────────────────────────────────────
  menu.addSubMenu(
    ui.createMenu('🔴 Strava / Reconexão')
      .addItem('🔗 Gerar links reconexão (atletas com erro)', 'gerarLinksReconexaoTodos')
      .addSeparator()
      .addItem('⚡ Importar atividades agora (todos)', 'importarAtividades')
      .addItem('👤 Importar perfil atleta (específico)', 'importarPerfilAtleta')
      .addSeparator()
      .addItem('📊 Status da fila Strava', 'statusFila')
      .addItem('🔄 Resetar contadores rate limit', 'resetarContadoresFila')
  );
    menu.addToUi();
}
