...
function setupPlanilha() {
  ...
  const backupStamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMdd_HHmmss');
  ABAS.forEach(nome => {
    const existente = ss.getSheetByName(nome);
    if (existente) {
      const backup = existente.copyTo(ss);
      backup.setName(('BACKUP_' + backupStamp + '_' + nome).substring(0, 99));
...
function _criarCadastro(ws) {
  ...
  const cols = [
    ...
    'Sexo','Peso (kg)','Altura (cm)','Modalidade','Nível','Objetivo',
    'Freq./Semana','Horário Pref.','Condições de Saúde','Histórico de Lesão',
    'Medicamentos','Prova/Meta','Plano','Cidade','Estado','CPF',
    'Canal de Origem','Data Cadastro','Strava Conectado',
  ...
  _dropdown(ws, 9,  3, 500, ['Corrida','Academia','Ergométrica','Híbrido']);
  _dropdown(ws, 10, 3, 500, ['Iniciante','Intermediário','Avançado']);
  _dropdown(ws, 11, 3, 500, ['Emagrecimento','Condicionamento','Performance','Saúde Mental','Força']);
  _dropdown(ws, 12, 3, 500, ['2x','3x','4x','5x']);
  _dropdown(ws, 13, 3, 500, ['Manhã','Tarde','Noite','Variável']);
  _dropdown(ws, 18, 3, 500, ['Experimental','Online R$100','Presencial R$120','Híbrido R$160']);
  _dropdown(ws, 22, 3, 500, ['Instagram','Indicação','Strava','Google','Outro']);
  _dropdown(ws, 24, 3, 500, ['Sim','Não','Pendente']);
  _dropdown(ws, 26, 3, 500, ['Ativo','Inativo','Suspenso','Trial']);

  // Formatação condicional Status
  _condTexto(ws, 'Z3:Z500', 'Ativo',    COR.verde_claro);
  _condTexto(ws, 'Z3:Z500', 'Inativo',  COR.vermelho_cl);
  _condTexto(ws, 'Z3:Z500', 'Trial',    COR.amarelo_cl);
  _condTexto(ws, 'Z3:Z500', 'Suspenso', '#E8E8E8');
  _condTexto(ws, 'X3:X500', 'Sim',      COR.verde_claro);
  _condTexto(ws, 'X3:X500', 'Não',      COR.vermelho_cl);

  const larguras = [12,22,24,16,12,9,9,11,13,13,18,11,12,22,22,18,16,20,16,10,14,16,14,15,12,10,26];
...
function _criarAtividades(ws) {
  ...
  _tituloPrincipal(ws, '🏃  ATIVIDADES — DADOS IMPORTADOS DO STRAVA', 24, COR.laranja);
  ...
  const cols = [
    ...
    'Cadência','Potência','Rota','Importado em'
  ];
  _cabecalho(ws, 2, cols, COR.cinza_escuro);
  _zebra(ws, 3, 502, 24, COR.branco, '#FFF5EE');
  _congelar(ws, 2, 4);
  ...
  const larguras = [13,11,18,12,12,9,13,22,14,13,12,12,13,14,14,14,10,10,11,10,10,10,24,16];
  larguras.forEach((w, i) => ws.setColumnWidth(i+1, w*7));
  ws.setRowHeight(2, 28);
  ws.setHiddenGridlines(true);
}
...
function _criarTokens(ws) {
  ...
  _tituloPrincipal(ws, '🔐  TOKENS STRAVA — NÃO COMPARTILHAR', 10, COR.cinza_escuro);

  const cols = ['ID Execução','ID Atleta','Nome Atleta','Access Token','Refresh Token','Expira Em (Unix)','Scope','ID Strava','Últ. Atualização','Status Token'];
  _cabecalho(ws, 2, cols, COR.preto);
  _zebra(ws, 3, 502, 10, COR.branco, COR.cinza_claro);
  _congelar(ws, 2, 2);

  for (let r = 3; r <= 502; r++) {
    ws.getRange(r, 1, 1, 10).setFontFamily('Courier New').setFontSize(9);
    ws.setRowHeight(r, 20);
  }

  _dropdown(ws, 10, 3, 502, ['Ativo','Renovado','Expirado','Revogado','Pendente']);
  _condTexto(ws, 'J3:J502', 'Ativo',    COR.verde_claro);
  _condTexto(ws, 'J3:J502', 'Renovado', COR.verde_claro);
  _condTexto(ws, 'J3:J502', 'Expirado', COR.vermelho_cl);
  _condTexto(ws, 'J3:J502', 'Revogado', '#E8E8E8');

  const larguras = [14,12,22,46,46,18,28,15,18,14];
  larguras.forEach((w, i) => ws.setColumnWidth(i+1, w*7));
  ws.setRowHeight(2, 28);
  ws.setHiddenGridlines(true);
}
