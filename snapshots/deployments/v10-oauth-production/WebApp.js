...
function doGet(e) {
  ...
  // Validação da inscrição de webhook Strava
  if (p['hub.mode'] === 'subscribe' && p['hub.challenge']) {
    return _validarWebhookStrava_(p);
  }
  ...
  // Conectar um atleta já cadastrado ao Strava sem refazer cadastro
  if (p.connect === 'strava') {
    const athId = String(p.athId || '').trim().toUpperCase();
    if (!athId) {
      return _paginaMensagemWA('ID inválido', 'Informe um ID de atleta para conectar ao Strava.', '#c00');
    }
    try {
      const atleta = _resolverAtleta(athId);
      return HtmlService.createHtmlOutput(_paginaRedirecionando(_gerarUrlOAuth(athId), atleta.nome))
        .setTitle('Conectando ao Strava...');
    } catch(errConnect) {
      _logErro('_connectStrava', athId, errConnect.message);
      return _paginaMensagemWA('Erro ao conectar Strava', errConnect.message, '#c00');
    }
  }
...
function _processarCallbackOAuth(e) {
  ...
  if (error) {
    _logErroSistema('_processarCallbackOAuth', 'Autorização recusada pelo atleta: ' + error);
  ...
  let estado;
  try {
    estado = _obterEstadoOAuth_(state);
  } catch (stateErr) {
    _logErroSistema('_processarCallbackOAuth', 'State OAuth inválido: ' + stateErr.message);
    return _paginaMensagemWA('Sessão de autorização inválida', stateErr.message, '#c00');
  }
  const athId = estado.athId;

  try {
    const scopeAceito = String(p.scope || '');
    if (scopeAceito && scopeAceito.indexOf('activity:read_all') < 0) {
      throw new Error('A permissão para ler todas as atividades não foi autorizada. Gere um novo link e mantenha essa permissão marcada.');
    }
    ...
    _atualizarStatusCadastro(athId, true, tokens.athlete && tokens.athlete.id);
    _consumirEstadoOAuth_(state);
    _log(athId, 'INFO', '_processarCallbackOAuth',
      'OAuth concluído para ' + estado.nome + ' | ID Strava: ' + (tokens.athlete && tokens.athlete.id || '') +
      ' | Scope: ' + (tokens.scope || scopeAceito), '');

    // Importação inicial para o aluno já aparecer com atividades após conectar.
    try { _importarAtividadesAtleta(athId, 1); } catch(ie) { _logErro('_importacaoInicialOAuth', athId, ie.message); }
