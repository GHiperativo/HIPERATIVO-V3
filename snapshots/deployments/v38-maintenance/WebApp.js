...
function doGet(e) {
  ...
  // ─── Endpoint de manutenção (execução remota sem UI) ────────────────────────
// Uso: ?manutencao=HIPERATIVO2026&fn=ajustesPlanilha
   // Uso: ?manutencao=HIPERATIVO2026&fn=setWebAppUrl
  if (p.manutencao === 'HIPERATIVO2026') {
      const novaUrl = 'https://script.google.com/macros/s/AKfycbzvJzeGQXtfpiRu0C3UI4gC7_9LRIJN0hTXZkR9h8hv3t66d4GTUpdwLIoWZk2Ke-4Mtg/exec';
  const props  = PropertiesService.getScriptProperties();
  
    if (p.fn === 'setWebAppUrl') {
      props.setProperty('WEBAPP_URL', novaUrl);
      props.setProperty('STRAVA_CLIENT_ID', '153043');
      return ContentService
        .createTextOutput(JSON.stringify({ ok: true, fn: 'setWebAppUrl', url: novaUrl }))
        .setMimeType(ContentService.MimeType.JSON);
    }

    if (p.fn === 'debug') {
      const info = {
        WEBAPP_URL:        props.getProperty('WEBAPP_URL')        || '(vazio)',
        STRAVA_CLIENT_ID:  props.getProperty('STRAVA_CLIENT_ID')  || '(vazio)',
        STRAVA_CLIENT_SECRET: props.getProperty('STRAVA_CLIENT_SECRET') ? '***ok***' : '(vazio)',
        SUPABASE_KEY:      props.getProperty('SUPABASE_KEY')      ? '***ok***' : '(vazio)',
    ...
    return _doManutencao(p.fn || 'ajustesPlanilha');
  }
  ...
  // Reconexão Strava: ?reconectar=ATH_ID
  // → se atleta existir no CADASTRO → redireciona direto para OAuth do Strava
  // → se não existir → mostra formulário de cadastro
  if (p.reconectar) {
    return _processarReconexao(p.reconectar.toUpperCase().trim());
  }
...
function _processarCallbackOAuth(e) {
  ...
  try {
    ...
    // Salvar tokens na planilha
        _salvarTokensPlanilha(athId, tokens);
...
// ── Manutenção remota via URL ──────────────────────────────────────────────────
/**
 * Executa funções de manutenção sem UI (chamado via doGet com ?manutencao=HIPERATIVO2026)
 * Retorna JSON com resultado de cada operação.
 */
function _doManutencao(fn) {
  const SHEET_ID = '1bI5pnt-HOAD5p8M2hqjEsU9P816hc94wy4mqx0J_xOM';
  const log      = [];

  try {
    const ss = SpreadsheetApp.openById(SHEET_ID);
    if (!ss) throw new Error('Planilha não encontrada: ' + SHEET_ID);

    // ── 1. ATIVIDADES: deletar linhas vazias (sem ATH_ID na col B) ──────────
    const shAtv = ss.getSheetByName('🏃 ATIVIDADES');
    if (shAtv) {
      const lastRow = shAtv.getLastRow();
      let delAtv = 0;
      if (lastRow > 1) {
        const athIds = shAtv.getRange(2, 2, lastRow - 1, 1).getValues();
        for (let i = athIds.length - 1; i >= 0; i--) {
          if (!String(athIds[i][0] || '').trim()) {
            shAtv.deleteRow(i + 2);
            delAtv++;
          }
        }
      }
      log.push('ATIVIDADES: ' + delAtv + ' linhas vazias removidas.');
    } else { log.push('ATIVIDADES: aba não encontrada.'); }

    // ── 2. TOKENS: deletar linhas de header corrompido ──────────────────────
    const shTok = ss.getSheetByName('🔐 TOKENS');
    if (shTok) {
      const lastTok = shTok.getLastRow();
      let delTok = 0;
      if (lastTok > 1) {
        const tokData = shTok.getRange(2, 1, lastTok - 1, 2).getValues();
        const patterns = ['TOK_ID', 'EXEC_ID', 'ID ATLETA', 'ID Atleta', 'NOME COMPLETO'];
        for (let i = tokData.length - 1; i >= 0; i--) {
          const v = String(tokData[i][0] || '').trim().toUpperCase();
          if (patterns.some(p => v.includes(p.toUpperCase()))) {
            shTok.deleteRow(i + 2);
            delTok++;
          }
        }
      }
      log.push('TOKENS: ' + delTok + ' linha(s) corrompida(s) removidas.');
    } else { log.push('TOKENS: aba não encontrada.'); }

    // ── 3. MÉTRICAS: deletar linhas de header duplicado ─────────────────────
    const shMet = ss.getSheetByName('📈 MÉTRICAS');
    if (shMet) {
      const lastMet = shMet.getLastRow();
      let delMet = 0;
      if (lastMet > 1) {
        const metData = shMet.getRange(2, 1, lastMet - 1, 2).getValues();
        const mpatterns = ['ID ATLETA', 'ATH_ID', 'ID Atleta'];
        for (let i = metData.length - 1; i >= 0; i--) {
          const v = String(metData[i][0] || '').trim().toUpperCase();
          if (mpatterns.some(p => v.includes(p.toUpperCase()))) {
            shMet.deleteRow(i + 2);
            delMet++;
          }
        }
      }
      log.push('MÉTRICAS: ' + delMet + ' linha(s) de header removidas.');
    } else { log.push('MÉTRICAS: aba não encontrada.'); }

    // ── 4. CONFIG: popular se vazia ─────────────────────────────────────────
    const shCfg = ss.getSheetByName('⚙️ CONFIG');
    if (shCfg) {
      if (shCfg.getLastRow() <= 1) {
        const params = [
          ['CHAVE','VALOR','DESCRIÇÃO'],
          ['NOME_GRUPO','Grupo Hiperativo','Nome do grupo'],
          ['EMAIL_TREINADOR','contato@ghiperativo.com.br','Email do treinador'],
          ['CIDADE','Brasília','Cidade sede'],
          ['ESTADO','DF','Estado'],
          ['DIAS_ALERTA_INATIVIDADE','14','Dias sem treino para alerta'],
          ['DIAS_ALERTA_MENSALIDADE','5','Dias antes do vencimento'],
          ['DIAS_ALERTA_AVALIACAO','90','Dias sem avaliação para alerta'],
          ['HORAS_ALERTA_FEEDBACK','48','Horas sem feedback visto'],
          ['SUPABASE_URL','https://korlpbclqgmqvpbrungc.supabase.co','URL Supabase'],
          ['SUPABASE_KEY','(configurar via menu Credenciais)','Não inserir aqui'],
          ['VERSAO_SISTEMA','V3.1.0','Versão atual'],
          ['STATUS_ATIVOS_VALIDOS','Ativo,Trial,Reativado','Status de atleta ativo'],
          ['SLOGAN','Cabeça • Coração • Corpo','Slogan do sistema'],
        ];
        shCfg.clearContents();
        shCfg.getRange(1, 1, params.length, 3).setValues(params);
        shCfg.getRange(1, 1, 1, 3).setBackground('#1a1a2e').setFontColor('#ffffff').setFontWeight('bold');
        shCfg.setFrozenRows(1);
        log.push('CONFIG: ' + (params.length - 1) + ' parâmetros inseridos.');
      } else {
        log.push('CONFIG: já tinha dados — mantida.');
      }
    } else { log.push('CONFIG: aba não encontrada.'); }

    // ── 5. ERROS: trimmar para últimas 200 linhas ────────────────────────────
    const shErr = ss.getSheetByName('🔴 ERROS');
    if (shErr) {
      const lastErr = shErr.getLastRow();
      if (lastErr > 201) {
        shErr.deleteRows(2, lastErr - 201);
        log.push('ERROS: trimado para 200 linhas.');
      } else {
        log.push('ERROS: ' + (lastErr - 1) + ' linhas — dentro do limite.');
      }
    } else { log.push('ERROS: aba não encontrada.'); }

    SpreadsheetApp.flush();

    const resultado = { ok: true, fn: fn, log: log, ts: new Date().toISOString() };
    return ContentService
      .createTextOutput(JSON.stringify(resultado))
      .setMimeType(ContentService.MimeType.JSON);

  } catch(e) {
    const erro = { ok: false, fn: fn, erro: e.message, log: log };
    return ContentService
...
// ── Reconexão Strava (?reconectar=ATH_ID) ─────────────────────────────────────
/**
 * Verifica se atleta tem cadastro:
 * - SIM → redireciona diretamente para OAuth do Strava (reconexão)
 * - NÃO → exibe formulário de cadastro para novo registro
 */
function _processarReconexao(athId) {
  if (!athId) {
    return _paginaMensagemWA('Link inválido', 'ID do atleta não informado.', '#c00');
  }
  ...
  try {
    ...
    if (!shCad) {
      return _paginaMensagemWA('Erro interno', 'Aba de cadastro não encontrada.', '#c00');
    }

    // Buscar atleta na aba CADASTRO
    const dados     = shCad.getDataRange().getValues();
    let atletaExiste = false;
    let nomeAtleta  = '';

    for (let i = 1; i < dados.length; i++) {
      const idCelula = String(dados[i][H.CAD.ID - 1] || '').trim().toUpperCase();
      if (idCelula === athId) {
        atletaExiste = true;
        nomeAtleta   = String(dados[i][H.CAD.NOME - 1] || '').trim();
        break;
      }
    }

    if (!atletaExiste) {
      // Atleta não tem cadastro → mostrar formulário de registro
      try {
        const tmpl  = HtmlService.createTemplateFromFile('cadastro');
        tmpl.athId  = '';
        return tmpl.evaluate()
        ...
        return HtmlService.createHtmlOutput(_paginaCadastro(''))
          .setTitle('Cadastro — Grupo Hiperativo')
          .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
      }
    }

    // Atleta existe → gerar URL OAuth do Strava e redirecionar
    const urlOAuth = _gerarUrlOAuth(athId);

    const html =
      '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
      '<meta name="viewport" content="width=device-width,initial-scale=1">' +
      '<meta http-equiv="refresh" content="2;url=' + urlOAuth + '">' +
      '<style>body{font-family:Arial,sans-serif;display:flex;align-items:center;' +
      'justify-content:center;min-height:100vh;margin:0;background:#f5f5f5;}' +
      '.card{background:#fff;padding:40px;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,.1);' +
      'max-width:480px;text-align:center;}' +
      'h2{color:#1a3a8a;margin-bottom:16px;}p{color:#444;line-height:1.6;}' +
      '.logo{font-size:48px;margin-bottom:16px;}' +
      '.btn{display:inline-block;margin-top:20px;padding:12px 28px;background:#FC4C02;' +
      'color:#fff;text-decoration:none;border-radius:6px;font-weight:bold;font-size:16px;}' +
      '</style></head><body><div class="card">' +
      '<div class="logo">⚡</div>' +
      '<h2>Reconectar ao Strava</h2>' +
      '<p>Olá' + (nomeAtleta ? ', <strong>' + nomeAtleta + '</strong>' : '') + '!</p>' +
      '<p>Redirecionando para o Strava em instantes...</p>' +
      '<a class="btn" href="' + urlOAuth + '">🔗 Conectar agora</a>' +
      '<p style="margin-top:24px;font-size:13px;color:#888">GRUPO HIPERATIVO<br>CABEÇA • CORAÇÃO • CORPO</p>' +
      '</div></body></html>';

    return HtmlService.createHtmlOutput(html)
      .setTitle('Reconectar Strava — Hiperativo')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
      .addMetaTag('viewport', 'width=device-width, initial-scale=1');

  } catch(err) {
    _logErro('_processarReconexao', athId, err.message);
    return _paginaMensagemWA('Erro', 'Não foi possível processar a reconexão: ' + err.message, '#c00');
  }
}
