...
function registrarAtividadesBrutasStrava_(athId, nomeAtleta, atividades) {
  ...
  atividades.forEach(function(act) {
    ...
    if (_rawIdsExecucao_.has(sid)) {
      var linhaLegada = _rawLegadoRowsExecucao_[sid];
      if (linhaLegada) {
        reparos.push({ row: linhaLegada, values: linhaReal });
