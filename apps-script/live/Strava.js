...
function _processarFormCadastro(p) {
  ...
  try {
    ...
    const duplicado = _encontrarCadastroDuplicado_(data, p, athId);
    if (duplicado) {
      throw new Error('Cadastro já existente para estes dados: ' + duplicado.athId +
        '. O registro atual foi preservado; fale com o treinador para atualizar seus dados.');
    }
    let rowIdx = -1;
    ...
    if (rowIdx !== -1) {
      const existente = data[rowIdx - 1];
      vals[21] = existente[H.CAD.DATA_CAD - 1] || vals[21];
      vals[22] = existente[H.CAD.STRAVA_OK - 1] || vals[22];
      vals[23] = existente[H.CAD.STRAVA_ID - 1] || vals[23];
      vals[24] = existente[H.CAD.STATUS - 1] || vals[24];
      vals[25] = existente[H.CAD.OBS - 1] || vals[25];
    }
    ...
    if (_temRefreshTokenValido_(athId)) {
      return HtmlService.createHtmlOutput(_paginaMensagemWA(
        'Cadastro atualizado',
        'Seus dados foram atualizados e sua conexão Strava foi preservada. Nenhuma nova autorização é necessária.',
        '#176b36'
...
function salvarCadastroAjax(p) {
  ...
  const lockCadastro = LockService.getScriptLock();
  if (!lockCadastro.tryLock(15000)) {
    return { ok: false, erro: 'Outro cadastro está sendo salvo agora. Aguarde alguns segundos e tente novamente.' };
  }
  let lockCadastroAtivo = true;
  ...
  try {
    ...
    const data = sheet.getDataRange().getValues();
    if (!athId) athId = _gerarAthIdComBase_(data);
    if (!_isAthIdValido_(athId)) return { ok: false, erro: 'Código de atleta inválido.' };

    const duplicado = _encontrarCadastroDuplicado_(data, p, athId);
    if (duplicado) {
      _log(duplicado.athId, 'AVISO', 'salvarCadastroAjax',
        'Tentativa de cadastro duplicado bloqueada; registro existente preservado.', duplicado.motivos.join(', '));
      return {
        ok: false,
        duplicado: true,
        athIdExistente: duplicado.athId,
        erro: 'Já existe um cadastro com estes dados (' + duplicado.athId + '). O registro anterior foi preservado.'
      };
    }
    let rowIdx = -1;
    for (let i = 1; i < data.length; i++) {
    ...
    if (rowIdx !== -1) {
      ...
      vals[24] = existente[H.CAD.STATUS - 1] || vals[24];
      vals[25] = existente[H.CAD.OBS - 1] || vals[25];
      vals[36] = existente[37] || vals[36];
      vals[37] = existente[38] || vals[37];
      vals[38] = existente[39] || vals[38];
    }
    ...
    lockCadastro.releaseLock();
    lockCadastroAtivo = false;
    ...
    const stravaJaConectado = usaStrava && _temRefreshTokenValido_(athId);
    let oauthUrl = '';
    if (usaStrava && !stravaJaConectado) {
    ...
    if (emailAtleta) {
      try {
        ...
        const corpo = _htmlEmailConfirmacaoCadastro(nomeAtleta, athId, oauthUrl, stravaJaConectado);
    ...
    try {
      if (usaStrava && !stravaJaConectado && typeof registrarAlertaStravaOperacional_ === 'function') {
        registrarAlertaStravaOperacional_(athId, 'PENDENTE_CADASTRO',
          'Cadastro concluído; autorização OAuth ainda não finalizada.', true);
      } else if (typeof resolverAlertasStravaOperacionais_ === 'function') {
        resolverAlertasStravaOperacionais_(athId);
      }
      if (typeof sincronizarVisoesAposInput_ === 'function') {
        sincronizarVisoesAposInput_('cadastro_web', athId, { permitirGlobal:false });
      }
    } catch (eVisual) {
      _log(athId, 'AVISO', 'salvarCadastroAjax', 'Atualização visual pendente: ' + eVisual.message, '');
    }
    ...
    return {
      ...
      oauthUrl: oauthUrl,
      usaStrava: usaStrava,
      stravaJaConectado: stravaJaConectado
    };
    ...
    if (lockCadastroAtivo) lockCadastro.releaseLock();
  }
}

function _temRefreshTokenValido_(athId) {
  try {
    const row = typeof _getTokenRow_ === 'function' ? _getTokenRow_(athId) : null;
    if (row && _isRefreshTokenValido_(String(row.refreshToken || ''))) return true;
    const props = PropertiesService.getScriptProperties();
    return _isRefreshTokenValido_(String(
      props.getProperty('RT_' + athId) || props.getProperty('TOK_REFRESH_' + athId) || ''
    ));
  } catch (_) {
    return false;
  }
...
/** Gera um identificador não sequencial e confirma que ele não existe na base. */
function _gerarAthIdComBase_(dadosCadastro) {
  const existentes = new Set((dadosCadastro || []).map(function(r) {
    return String(r[H.CAD.ID - 1] || '').trim().toUpperCase();
  }));
  let id = '';
  do {
    id = 'ATH' + Utilities.getUuid().replace(/-/g, '').substring(0, 8).toUpperCase();
  } while (existentes.has(id));
  return id;
}

function _normalizarEmailCadastro_(valor) {
  return String(valor || '').trim().toLowerCase();
}

function _normalizarTelefoneCadastro_(valor) {
  const digitos = String(valor || '').replace(/\D/g, '');
  return digitos.length >= 10 ? digitos.slice(-11) : '';
}

/**
 * Bloqueia somente colisões inequívocas. Nunca mescla linhas nem altera o
 * cadastro anterior automaticamente.
 */
function _encontrarCadastroDuplicado_(dados, p, athIdAtual) {
  const email = _normalizarEmailCadastro_(p && p.email);
  const telefone = _normalizarTelefoneCadastro_(p && p.whats);
  const idAtual = String(athIdAtual || '').trim().toUpperCase();
  for (let i = 2; i < (dados || []).length; i++) {
    const row = dados[i];
    const id = String(row[H.CAD.ID - 1] || '').trim().toUpperCase();
    if (!_isAthIdValido_(id)) continue;
    const motivos = [];
    const emailExistente = _normalizarEmailCadastro_(row[H.CAD.EMAIL - 1]);
    const telefoneExistente = _normalizarTelefoneCadastro_(row[H.CAD.WHATS - 1]);
    if (id === idAtual) {
      if (email && emailExistente && emailExistente !== email) motivos.push('ATH_ID pertence a outro email');
      if (telefone && telefoneExistente && telefoneExistente !== telefone) motivos.push('ATH_ID pertence a outro whatsapp');
    } else {
      if (email && emailExistente === email) motivos.push('email');
      if (telefone && telefoneExistente === telefone) motivos.push('whatsapp');
    }
    if (motivos.length) return { athId: id, linha: i + 1, motivos: motivos };
  }
  return null;
}

// ── Template email confirmacao de cadastro ─────────────────────────────────────
function _htmlEmailConfirmacaoCadastro(nome, athId, linkStrava, stravaJaConectado) {
  const blocoStrava = stravaJaConectado ? `
  <hr style="border:none;border-top:1px solid #eee;margin:24px 0">
  <h3 style="color:#176b36;margin:0 0 8px">✅ Strava já conectado</h3>
  <p style="color:#444;line-height:1.6">Sua conexão existente e o refresh token foram preservados. Você não precisa autorizar novamente.</p>` : linkStrava ? `
  <hr style="border:none;border-top:1px solid #eee;margin:24px 0">
...
function _gravarAtividades(athId, nomeAtleta, atividades, opcoes) {
  opcoes = opcoes || {};
  ...
  try {
    if (typeof registrarAtividadesBrutasStrava_ === 'function') {
      registrarAtividadesBrutasStrava_(athId, nomeAtleta, atividades, opcoes);
    }
  } catch (eRaw) {
  ...
  for (const a of atividades) {
    ...
    const linha = [
      ...
      norm.status || 'Importado',                // 34 STATUS / qualidade
    ];
    ...
    if (linhaExistente) {
      ...
      if (!opcoes.forcarAtualizacao && !_atividadePrecisaReparo_(atual, linha)) continue;
...
function _atividadePrecisaReparo_(linha, oficial) {
  ...
  return !String(linha[2] || '').trim() ||
    ...
    !/Qualidade OK|⚠️/.test(String(linha[33] || '')) ||
...
function triggerImportacaoAutomatica() {
  ...
  // Recupera eventos que o caminho em tempo real guardou no Supabase, mas não
  // conseguiu concluir. Essa reconciliação diária é a rede de segurança.
  try {
    if (typeof processarFilaWebhookSupabase === 'function') processarFilaWebhookSupabase();
  } catch (e) {
    _log('SISTEMA', 'AVISO', 'triggerImportacaoAutomatica',
      'Fila de segurança do webhook: ' + e.message, '');
  }

  // Enquanto o webhook estiver ativo, acionadores antigos de outras contas
  // podem continuar instalados sem repetir a varredura pesada. A fila de
  // segurança acima ainda é consultada em todas as chamadas; o ciclo completo
  // fica limitado a uma execução por dia em todo o projeto.
  try {
    if (typeof _rtDeveExecutarReconciliacaoCompleta_ === 'function' &&
        !_rtDeveExecutarReconciliacaoCompleta_()) {
      _log('SISTEMA', 'INFO', 'triggerImportacaoAutomatica',
        'Webhook ativo; ciclo pesado já executado nas últimas 20 horas.', '');
      return;
    }
  } catch (e) {
    _log('SISTEMA', 'AVISO', 'triggerImportacaoAutomatica',
      'Controle de reconciliação indisponível; ciclo preservado: ' + e.message, '');
  }
  ...
  try {
    if (typeof sincronizarCadastrosSHEPendentes === 'function') {
      sincronizarCadastrosSHEPendentes();
    }
...
function _atualizarStatusCadastro(athId, conectado, stravaId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][H.CAD.ID - 1] || '').trim().toUpperCase() === athId.toUpperCase()) {
      // "Não" significa que o atleta declarou não usar Strava. Uma revogação
      // confirmada é outro estado e deve aparecer como "Reconectar".
      sheet.getRange(i + 1, H.CAD.STRAVA_OK).setValue(conectado ? 'Sim' : 'Reconectar');
      ...
      if (typeof atualizarStatusConexaoStravaAtleta === 'function') {
        atualizarStatusConexaoStravaAtleta(athId, {
          forcarStatus: conectado ? 'Conectado' : 'Reconectar',
          atualizarFila: true,
          origem: conectado ? 'oauth_ou_refresh' : 'revogacao_confirmada'
        });
      }
      return;
    }
...
function persistirCredenciaisStrava(athId, tokenData, origem) {
  ...
  const outroDono = _encontrarDonoStravaId_(tokenData.stravaId, athId);
  if (outroDono) {
    _logEvento_('ERRO', 'persistirCredenciaisStrava', athId,
      'ID Strava já vinculado a ' + outroDono + '; gravação bloqueada para preservar o vínculo existente', origem);
    return false;
  }
  ...
  const persistido = salvo || backupLocalSalvo || supabaseSalvo;
  // Só uma persistência realmente concluída invalida um marcador anterior de
  // revogação. As cópias de refresh_token permanecem intactas.
  if (persistido) {
    try {
...
/** Evita que uma mesma conta Strava seja ligada silenciosamente a dois ATH_IDs. */
function _encontrarDonoStravaId_(stravaId, athIdAtual) {
  const alvo = String(stravaId || '').trim();
  if (!alvo) return '';
  try {
    const ss = SpreadsheetApp.openById(_getSsId());
    const abas = [
      { sh: ss.getSheetByName(H.SHEETS.TOKENS), idCol: H.TOK.ATH_ID, stravaCol: H.TOK.STRAVA_ID, inicio: 1 },
      { sh: ss.getSheetByName(H.SHEETS.CADASTRO), idCol: H.CAD.ID, stravaCol: H.CAD.STRAVA_ID, inicio: 3 }
    ];
    for (let a = 0; a < abas.length; a++) {
      const cfg = abas[a];
      if (!cfg.sh) continue;
      const rows = cfg.sh.getDataRange().getValues();
      for (let i = cfg.inicio; i < rows.length; i++) {
        const id = String(rows[i][cfg.idCol - 1] || '').trim().toUpperCase();
        const sid = String(rows[i][cfg.stravaCol - 1] || '').trim();
        if (_isAthIdValido_(id) && id !== String(athIdAtual || '').trim().toUpperCase() && sid === alvo) return id;
      }
    }
  } catch (e) {
    _logEvento_('AVISO', '_encontrarDonoStravaId_', athIdAtual,
      'Não foi possível concluir a verificação de duplicidade Strava', e.message);
  }
  return '';
}
...
function verificarStatusStravaAtletas() {
  // Fonte única de verdade. Atualiza primeiro o cadastro operacional (AX/AY)
  // e depois reconstrói o painel oculto sem tocar nos tokens.
  const resumoStatus = typeof sincronizarStatusConexaoStravaTodos === 'function'
    ? sincronizarStatusConexaoStravaTodos({ atualizarFila: false })
    : { atualizados: 0 };
  if (typeof atualizarStravaStatusSheet === 'function') atualizarStravaStatusSheet();
  return resumoStatus;
...
function migrarTokenScriptPropertiesParaTokensESupabase(athId) {
  ...
  if (precisaRenovar) {
    ...
    const supaUrl = props.getProperty('SUPABASE_URL') || 'https://korlpbclqgmqvpbrungc.supabase.co';
    const clientId = props.getProperty('STRAVA_CLIENT_ID');
...
function _t1_auditarTriggers() {
  // Compatibilidade com a auditoria antiga: agora é estritamente somente
  // leitura. Nenhum acionador pode ser apagado por uma função de diagnóstico.
  const triggers = ScriptApp.getProjectTriggers();
  const resultado = triggers.map(function(t) {
    return {
      fn: t.getHandlerFunction(),
      src: t.getTriggerSource().toString(),
      evt: t.getEventType().toString(),
      status: 'MANTIDO — auditoria somente leitura'
    };
  });
