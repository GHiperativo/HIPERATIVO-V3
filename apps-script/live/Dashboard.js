...
function atualizarStravaStatusSheet() {
  ...
  tokenPorAtleta.forEach(function(item, athId) {
    ...
    const refreshToken = String(tok[H.TOK.REFRESH - 1] || '').trim();
    ...
    // Status operacional: o refresh token, e não a expiração temporária do
    // access token, é o que define se a conexão pode continuar sem pedir nova
    // autorização ao atleta.
    const revogado = !!PropertiesService.getScriptProperties()
      .getProperty('STRAVA_REVOGADO_' + athId);
    let temRefresh = typeof _isRefreshTokenValido_ === 'function'
      ? _isRefreshTokenValido_(refreshToken)
      : refreshToken.length >= 20;
    if (!temRefresh && typeof _temRefreshTokenValido_ === 'function') {
      try { temRefresh = _temRefreshTokenValido_(athId); } catch (_) {}
    }
    let statusLabel = 'Aguardando conexão';
    if (naoUsaStrava) statusLabel = 'Não utiliza';
    else if (revogado) statusLabel = 'Reconectar';
    else if (temRefresh) statusLabel = 'Conectado';
    ...
    else if (statusLabel === 'Reconectar') obs = 'Revogação confirmada pela Strava — reconectar';
    else if (statusLabel === 'Aguardando conexão') obs = 'Conta declarada, mas OAuth ainda não foi concluído';
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    let statusBg = bg;
    if (status === 'Conectado')    statusBg = corVerde;
    else if (status === 'Não utiliza') statusBg = '#EEEEEE';
    else if (status === 'Reconectar') statusBg = corVerm;
