...
function normalizarAtividadeStrava(activity, atleta) {
  ...
  normalizada.status = _statusQualidadeAtividade_(normalizada);
  return normalizada;
}

/**
 * Sinaliza dados improváveis sem excluir nem bloquear a atividade. Os limites
 * são de integridade operacional, não diagnóstico fisiológico.
 */
function _statusQualidadeAtividade_(n) {
  var alertas = [];
  var data = n && n.data_hora instanceof Date ? n.data_hora : null;
  var agoraMais24h = Date.now() + 24 * 60 * 60 * 1000;
  if (!data || isNaN(data.getTime())) alertas.push('data ausente/inválida');
  else if (data.getTime() > agoraMais24h) alertas.push('data futura');

  var mov = Number(n && n.tempo_mov_s);
  var total = Number(n && n.tempo_total_s);
  var dist = Number(n && n.dist_km);
  var pace = Number(n && n.pace_s_km);
  var fcMed = Number(n && n.fc_media);
  var fcMax = Number(n && n.fc_max);
  var cad = Number(n && n.cadencia);
  var tipoOriginal = String(n && n.tipo_original || '');
  var ehCorrida = typeof TIPOS_CORRIDA !== 'undefined' && TIPOS_CORRIDA.includes(tipoOriginal);

  if (!(mov > 0)) alertas.push('tempo de movimento zerado');
  if (total > 0 && mov > total + 5) alertas.push('tempo de movimento maior que total');
  if (dist < 0) alertas.push('distância negativa');
  if (ehCorrida && dist >= 0.5 && pace > 0 && (pace < 120 || pace > 1800)) alertas.push('pace fora da faixa operacional');
  if (fcMed && (fcMed < 30 || fcMed > 230)) alertas.push('FC média fora da faixa operacional');
  if (fcMax && (fcMax < 30 || fcMax > 250)) alertas.push('FC máxima fora da faixa operacional');
  if (fcMed && fcMax && fcMax < fcMed) alertas.push('FC máxima menor que FC média');
  if (cad && ehCorrida && (cad < 35 || cad > 250)) alertas.push('cadência fora da faixa operacional');

  return alertas.length ? 'Importado | ⚠️ ' + alertas.join('; ') : 'Importado | Qualidade OK';
}

/** Reclassifica apenas a coluna STATUS; valores, PSE e atividades são preservados. */
function reclassificarQualidadeAtividades() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sh = ss.getSheetByName(H.SHEETS.ATIVIDADES);
  if (!sh || sh.getLastRow() < 3) return { processadas: 0, alertas: 0 };
  const rows = sh.getRange(3, 1, sh.getLastRow() - 2, 34).getValues();
  let alertas = 0;
  const status = rows.map(function(r) {
    const n = {
      data_hora: r[H.ATIV.DATA - 1],
      tempo_mov_s: r[H.ATIV.MOV_S - 1],
      tempo_total_s: r[H.ATIV.TOTAL_S - 1],
      dist_km: r[H.ATIV.DIST_KM - 1],
      pace_s_km: r[H.ATIV.PACE_S - 1],
      fc_media: r[H.ATIV.FC_MED - 1],
      fc_max: r[H.ATIV.FC_MAX - 1],
      cadencia: r[H.ATIV.CADENCIA - 1],
      tipo_original: r[H.ATIV.TIPO_ORIG - 1] || r[H.ATIV.TIPO - 1]
    };
    const s = _statusQualidadeAtividade_(n);
    if (s.indexOf('⚠️') >= 0) alertas++;
    return [s];
  });
  sh.getRange(3, H.ATIV.STATUS, status.length, 1).setValues(status);
  _log('SISTEMA', 'INFO', 'reclassificarQualidadeAtividades',
    status.length + ' atividades classificadas; ' + alertas + ' alerta(s) sem descarte.', '');
  return { processadas: status.length, alertas: alertas };
}
