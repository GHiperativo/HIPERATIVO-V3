...
function corrigirAbaMetricas(ss) {
  ...
  _formatarMetricas_(ws, Math.max(1, ws.getLastRow() - 2));
  return;
...
/**
 * Executa somente os ajustes aprovados e reversíveis da auditoria Etapas 1/2.
 * Não apaga linhas, abas, propriedades de token ou cursores de histórico.
 */
function aplicarAuditoriaEtapas12Segura() {
  const antes = _contarRefreshTokensPlanilha_();
  const resultado = { refreshAntes: antes, etapas: {} };

  resultado.etapas.supabaseUrl = _corrigirSupabaseUrl();
  resultado.etapas.triggers = configurarTriggers(true);
  resultado.etapas.qualidade = reclassificarQualidadeAtividades();

  try { resultado.etapas.metricas = calcularMetricasTodos(true); }
  catch (e) { resultado.etapas.metricas = 'ERRO: ' + e.message; }
  try { resultado.etapas.analise = atualizarAnaliseSheet(); }
  catch (e) { resultado.etapas.analise = 'ERRO: ' + e.message; }
  try { resultado.etapas.ranking = atualizarRankingSheet(); }
  catch (e) { resultado.etapas.ranking = 'ERRO: ' + e.message; }
  try { resultado.etapas.painel = atualizarPainel(); }
  catch (e) { resultado.etapas.painel = 'ERRO: ' + e.message; }
  try { resultado.etapas.abas = organizarAbasOperacionais(); }
  catch (e) { resultado.etapas.abas = 'ERRO: ' + e.message; }

  const depois = _contarRefreshTokensPlanilha_();
  resultado.refreshDepois = depois;
  resultado.tokensPreservados = depois >= antes;
  if (!resultado.tokensPreservados) {
    _log('SISTEMA', 'ERRO', 'aplicarAuditoriaEtapas12Segura',
      'Contagem de refresh tokens diminuiu de ' + antes + ' para ' + depois + '.', '');
    throw new Error('Verificação de segurança falhou: a contagem de refresh tokens diminuiu.');
  }
  _log('SISTEMA', 'INFO', 'aplicarAuditoriaEtapas12Segura',
    'Auditoria aplicada; refresh tokens preservados (' + antes + ' → ' + depois + ').', '');
  Logger.log('AUDITORIA_ETAPAS_1_2 ' + JSON.stringify(resultado));
  return resultado;
}

/** Diagnóstico final sem expor o conteúdo de nenhuma credencial. */
function diagnosticarAuditoriaEtapas12() {
  ...
  if (shAtiv && shAtiv.getLastRow() >= 3) {
    const status = shAtiv.getRange(3, H.ATIV.STATUS, shAtiv.getLastRow() - 2, 1).getValues();
    atividades = status.length;
    alertasQualidade = status.filter(function(r) { return String(r[0] || '').indexOf('⚠️') >= 0; }).length;
  }
  const props = PropertiesService.getScriptProperties();
  const resultado = {
    refreshTokensValidos: _contarRefreshTokensPlanilha_(),
    supabaseUrlCorreta: props.getProperty('SUPABASE_URL') === 'https://korlpbclqgmqvpbrungc.supabase.co',
    triggers: diagnosticarTriggersEssenciais(),
    atividades: atividades,
    alertasQualidade: alertasQualidade,
    verificadoEm: new Date().toISOString()
  };
  Logger.log('DIAGNOSTICO_AUDITORIA ' + JSON.stringify(resultado));
  return resultado;
}

function _contarRefreshTokensPlanilha_() {
  const ss = SpreadsheetApp.openById(_getSsId());
  const sh = ss.getSheetByName(H.SHEETS.TOKENS);
  if (!sh || sh.getLastRow() < 2) return 0;
  return sh.getRange(1, H.TOK.REFRESH, sh.getLastRow(), 1).getValues()
    .filter(function(r) { return _isRefreshTokenValido_(String(r[0] || '')); }).length;
}
