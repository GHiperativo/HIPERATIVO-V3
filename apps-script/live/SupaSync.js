  ...
  try {
    ...
    candidatos.forEach(function(a) {
      ...
      try {
        ...
        const revogacaoConfirmada = !!PropertiesService.getScriptProperties()
          .getProperty('STRAVA_REVOGADO_' + athId);
        if (revogacaoConfirmada) {
          confirmados.push(Object.assign({}, a, {
            _motivo: 'desautorização confirmada pela própria Strava'
          }));
          _log(athId, 'AVISO', 'monitorarStravaOk',
            'Reconexão confirmada por evento oficial de desautorização da Strava; tokens preservados', '');
          return;
        }
        ...
        if (!temRefresh) {
          confirmados.push(Object.assign({}, a, {
            _motivo: 'refresh_token não encontrado nas fontes de segurança'
          }));
          _log(athId, 'AVISO', 'monitorarStravaOk',
    ...
    const linhas = confirmados.map(a =>
      '• ' + a.nome + ' (' + a.ath_id + ') — ' +
        (a._motivo || 'reconexão confirmada')
    ).join('\n');

    const corpo = [
      '⚠️ ALERTA HIPERATIVO V3 — reconexão confirmada',
      '',
      confirmados.length + ' atleta(s) com reconexão realmente necessária:',
      '',
      linhas,
      '',
      'O monitor verificou Planilha, armazenamento interno, Supabase e eventos oficiais da Strava.',
...
// ─── COMPATIBILIDADE COM O ACIONADOR ANTIGO DE STATUS ─────────────────────────
// Esta rotina escrevia "✅ Ativo", "🔄 Só Supabase" e "❌ Reconectar" na coluna
// de declaração de uso do Strava, cuja validação aceita apenas Sim, Não,
// Pendente e Reconectar. O acionador criado por outra conta não pode ser
// excluído por este usuário; por isso o nome é preservado como uma função
// inofensiva. O estado real continua sendo atualizado pelos fluxos de OAuth,
// renovação de tokens e monitorarStravaOk(), sem alterar refresh tokens.

function atualizarStatusStravaEmCadastro() {
  Logger.log(
    'Rotina legada desativada com segurança. Status mantido pelos fluxos atuais de Strava.'
  );
  return {
    ok: true,
    desativado: true,
    substituidoPor: 'OAuth + renovacaoProativaTokens + monitorarStravaOk'
  };
}

// Remove somente acionadores pertencentes à conta que executar esta função e
// não cria outro acionador para a rotina legada.
...
function instalarAcionadorStatusCadastro() {
  ...
  Logger.log('Rotina legada sem acionador. Removidos nesta conta: ' + removidos.length);
  return { ok: true, removidos: removidos.length, criado: false };
}
